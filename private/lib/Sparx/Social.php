...
class Sparx_SocialConnect
{
    ...
    public function login($service, $callbackUrl, $request, $redirector)
    {
        switch($service) {
            case 'twitter':
            ...
            case 'facebook':
                ...
                if(!isset($query['code'])) {
                    ...
                    '&scope=publish_stream,offline_access,user_birthday';
            ...
            case 'hyves':
                ...
                break;
            case 'email':
            	$session = new Zend_Session_Namespace('social.email');
            	
            	$auth = Zend_Auth::getInstance();
            	$adapter = new My_Auth_Adapter( $request->getParam('email'), $request->getParam('password') );
            	
            	if( $adapter->authenticate() )
            	{
	            	$session->user_id = $adapter->getUserId();
            	}else{
            		
            		$session->message = $adapter->getMessage();
            		$redirector->gotoRouteAndExit( array(), 'login');
            	}
