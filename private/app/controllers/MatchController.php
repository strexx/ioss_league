...
class MatchController extends AppController {
    ...
    {   
        ...
        // Home positions
        $q = $db->prepare('SELECT * FROM match_positions WHERE match_id = :match_id AND team = :team ORDER BY position');
        $q->bindValue(':match_id', $matchId);
        $q->bindValue(':team', 'home');
        $q->execute();
        $hPositions = $q->fetchAll(PDO::FETCH_ASSOC);

        // Away positions
        $q = $db->prepare('SELECT * FROM match_positions WHERE match_id = :match_id AND team = :team ORDER BY position');
        $q->bindValue(':match_id', $matchId);
        $q->bindValue(':team', 'away');
        $q->execute();
        $aPositions = $q->fetchAll(PDO::FETCH_ASSOC);

		// Views
        $this->view->matchInfo = $matchInfo;
        $this->view->events = $events;
        $this->view->hPositions = $hPositions;
        $this->view->aPositions = $aPositions;
