...
if(isset($_SESSION['SimpleImageManager']) AND count($_SESSION['SimpleImageManager']) > 0){
	...
	foreach($_SESSION['SimpleImageManager'] as $s){
		...
		if($me){
			$html .= '<a style="margin: 9px; margin-right: 27px;" href="" class="img-thumbs" rel="' .$s . '"><img src="timthumb.php?src=' . $s . '&w=130&h=90" class="img-polaroid" width="130" height="90"></a>';
		}elseif($exists){
			$html .= '<a style="margin: 9px; margin-right: 27px;" href="" class="img-thumbs" rel="' .$s . '"><img src="' . $s . '" class="img-polaroid" width="130" height="90"></a>';
		}
	}
	if($html != ''){
		$output["html"] = $html;
